...
export default function App() {
  ...
  const straightTableTotalWidth = useMemo(() => {
    return straightTableColumns.reduce(
      (sum, _column, index) => sum + getStraightTableColumnWidth(index),
      0
    );
  }, [straightTableColumns]);
  ...
  async function loadAllWorksheetRowsForExport(
    ...
    try {
      ...
      try {
        const fullData = await reader.getAllPagesAsync(0);
        const fields = buildFieldMeta(fullData.columns);
        const rows = dataTableToRows(fullData, worksheet.name);
        return { rows, fields, totalRowCount };
      } catch {
  ...
  function buildStraightExportRowsFromRows(rows: PreviewRow[]) {
    return rows.map((row) => {
  ...
  function buildPivotTreeFromRows(rows: PreviewRow[]): PivotNode[] {
    if (selectedDimensions.length === 0) {
      return [];
    }

    function buildLevel(
      levelRows: PreviewRow[],
      dimensionIndex: number,
      parentId?: string,
      parentPath = ''
    ): PivotNode[] {
  ...
  }, [hasMorePages, isLoadingNextPage, selectedSources]);
          ...
          >
            {gridWidth > 0 && gridHeight > 0 && straightTableColumns.length > 0 ? (
              <Grid
                ...
                cellComponent={StraightGridCell as any}
